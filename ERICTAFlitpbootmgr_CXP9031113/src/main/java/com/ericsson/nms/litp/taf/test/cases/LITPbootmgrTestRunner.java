...
public class LITPbootmgrTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPbootmgrTestRunner.class);
    ...
    @TestId(id = "CXP9031113-1", title = "Upgrade ERIClitpbootmgr rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpbootmgrRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("bootmgr");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031113-2", title = "Run python test cases for ERIClitpbootmgr package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpbootmgrTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpbootmgrTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031113-3", title = "Parse xml outputs from python tests")
